...
setInterval(async () => {
	const resp = await fetch('https://api-public.odpt.org/api/v4/gtfs/realtime/odpt_HakodateCity_Alllines_vehicle')
	if (!resp.ok) {
...
setInterval(async () => {
	const resp = await fetch('https://api-public.odpt.org/api/v4/gtfs/realtime/odpt_HakodateCity_Alllines_trip_update')
	if (!resp.ok) {
		throw new Error(`${resp.status}`)
	}
	const buffer = await resp.arrayBuffer()
